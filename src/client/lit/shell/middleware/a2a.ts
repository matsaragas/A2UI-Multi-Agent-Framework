            ...
            req.on("end", async () => {
              ...
              if ("error" in response) {
                ...
                const result = (response as SendMessageSuccessResponse)
                  .result as Task;
                if (result.kind === "task") {
                  let parts: Part[] = result.status.message?.parts ?? [];

                  // If no parts in status message, check artifacts
                  if (parts.length === 0 && (result as any).artifacts?.length > 0) {
                  ...
                  // If still no parts, check history
                  if (parts.length === 0 && (result as any).history?.length > 0) {
                    const history = (result as any).history;
                    for (let i = history.length - 1; i >= 0; i--) {
                      if (history[i].role === 'agent' && history[i].parts) {
                        parts.push(...history[i].parts);
                        break;
                      }
                    }
                  }
