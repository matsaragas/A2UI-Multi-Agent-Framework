...
import { config as universalConfig } from "./configs/universal.js";
...
export class A2UILayoutEditor extends SignalWatcher(LitElement) {
  ...
  @state()
  accessor #history: Array<{
    id: string;
    processor: v0_8.A2uiMessageProcessor;
    messages: v0_8.Types.ServerToClientMessage[];
    query: string;
  }> = [];

  @state()
  accessor config: AppConfig = configs.universal;
  ...
  connectedCallback() {
    ...
    this.config = (appKey && configs[appKey]) || configs.universal;
    ...
    return html`
      ${!hasData && this.config.heroImage
        ? html`<div
          ...
          ></div>`
        : nothing}
      <h1 class="app-title" style="padding: 16px; text-align: center; margin: 0; font-size: ${hasData ? "1.5rem" : "2rem"}">
        ${this.config.title}
      </h1>
    `;
  }
  ...
  #renderHistory() {
    ...
    return html`<section id="surfaces">
      ...
      (interaction) => {
        return html`<div class="interaction">
            <div class="query-text">${interaction.query}</div>
            ${repeat(
          interaction.processor.getSurfaces(),
          ([surfaceId]) => surfaceId,
          ([surfaceId, surface]) => {
                ...
                if (evt.detail.action.context) {
                  ...
                  for (const item of srcContext) {
                    if (item.value.literalBoolean) {
                      ...
                      const path = interaction.processor.resolvePath(
                        item.value.path,
                        evt.detail.dataContextPath
                      );
                      const value = interaction.processor.getData(
  ...
  async #sendAndProcessMessage(request: v0_8.Types.A2UIClientEventMessage) {
    let query = request.userAction.name;
    if (request.userAction.context?.label) {
      query = request.userAction.context.label as string;
    }
